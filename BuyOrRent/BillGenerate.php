...
//    if(mysqli_num_rows($qer1)>0) {
   ...
   //    echo "res = $res['Quantity']";
        if (($res['Quantity'] - $qt) < 0) {
            echo "QUANTITY CHOSEN IS LESS THAN THE AVAILABLE QUANTITY!";
        }
//    }
    else {
...
//    else
        ...
        $qer6=mysqli_query($d,"Select Cost from Machinery_Details where Equipment_ID='$fq[2]'");
        $ressss=mysqli_fetch_array($qer6);
        echo "<h2>BILL RECEIPT</h2><table ><tr><th>Farmer ID</th><th>Equipment Name</th><th>Piece(s)</th><th>Dealer Name</th><th>Cost(Rs)</th><th>Date</th></tr><tr><td>$farmid</td><td>$ress[Equipment_Name]</td><td>$qt</td><td>$resss[Dealer_Name]</td><td>$ressss[Cost]</td><td>$dt</td></tr>";
...
?>
    <br><br><center><p  style=" text-align:right;"><a href="/Testproject/start.html">Home</a></p></center>
</body></html>